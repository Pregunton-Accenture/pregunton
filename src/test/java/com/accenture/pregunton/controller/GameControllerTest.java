...
public class GameControllerTest {
  ...
  public void deleteGame_WhenDeletingGame_ThenReturn204() throws Exception {
    gameService.delete(ModelUtil.ID); Mockito.verify(gameService, Mockito.times(1))
        .delete(ModelUtil.ID);
  ...
  @Test
  public void getGameQuestionsWithoutAnswer_WhenSendingValidCodeOfAGame_ShouldReturn200() throws Exception {

    Mockito.when(gameService.obtainQuestionsWithoutAnswer(ModelUtil.CODE))
        .thenReturn(Stream.of(ModelUtil.QUESTION_DTO)
            .collect(Collectors.toList()));

    mvc.perform(get("/games/v1.0/game/code/{code}", ModelUtil.CODE).contentType(MediaType.APPLICATION_JSON)
        .characterEncoding("utf-8")
  ...
  @Test
  public void getGameQuestionsWithoutAnswer_WhenSendingInvalidCodeOfAGame_ShouldThrowGameNotFoundException()
      throws Exception {

    Mockito.when(gameService.obtainQuestionsWithoutAnswer(any()))
  ...
  @Test
  public void getAllGameQuestions_WhenSendingValidCode_ShouldReturn200() throws Exception {
    Mockito.when(gameService.obtainAllQuestions(ModelUtil.CODE))
        .thenReturn(Collections.singletonList(ModelUtil.QUESTION_DTO));

    mvc.perform(get("/games/v1.0/game/code/{code}/all", ModelUtil.ID).contentType(MediaType.APPLICATION_JSON)
        .characterEncoding("utf-8")
        .content(objectMapper.writeValueAsString(Collections.singletonList(ModelUtil.QUESTION_DTO)))
        .header("code", ModelUtil.CODE))
        .andExpect(status().isOk());
