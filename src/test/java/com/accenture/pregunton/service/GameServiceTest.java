...
public class GameServiceTest {
  ...
  public void shouldCreateNewGame() {
    ...
    Mockito.when(modelMapper.map(any(), eq(Game.class)))
        .thenReturn(game); Mockito.when(categoryRepository.findById(ModelUtil.ID))
  ...
  public void getOne_WhenIDExists_ShouldReturnGameDto() {
    ...
    Assert.assertTrue(result.isPresent()); Assert.assertEquals(ModelUtil.GAME_DTO, result.get());
  }
  ...
  public void shouldAddAPlayerIfThereIsNone() {
    Game game = Game.builder()
        .build(); Mockito.when(gameRepository.findById(ModelUtil.ID))
  ...
  @Test
  public void obtainQuestionsWithoutAnswer_WhenSendingValidGameCode_ShouldReturnAListOfQuestions() {
    ...
    gameService.obtainQuestionsWithoutAnswer(ModelUtil.CODE);
  }

  @Test(expected = GameNotFoundException.class)
  public void obtainQuestionsWithoutAnswer_WhendSendingInvalidGameCode_ShouldThrowGameNotFoundException() {
    Mockito.when(gameRepository.findById(ModelUtil.ID))
        .thenReturn(Optional.empty());
    gameService.obtainQuestionsWithoutAnswer(ModelUtil.CODE);
  }
  ...
  @Test
  public void obtainAllQuestions_WhenSendingValidGameCode_ShouldReturnAlistOfQuestions() {
    Mockito.when(gameRepository.findByCode(ModelUtil.CODE))
        .thenReturn(Optional.of(ModelUtil.GAME));
    Mockito.when(mapperList.mapToDtoList(Stream.of(ModelUtil.QUESTION)
        .collect(Collectors.toList()), o -> modelMapper.map(o, QuestionDto.class)))
        .thenReturn(Stream.of(ModelUtil.QUESTION_DTO)
            .collect(Collectors.toList()));
    gameService.obtainAllQuestions(ModelUtil.CODE);
  }
