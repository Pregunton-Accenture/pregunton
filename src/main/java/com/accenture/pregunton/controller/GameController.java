...
public class GameController {
  ...
  @ApiOperation("get only the questions that are not anwered yet.")
  @ApiResponses(value = {
      @ApiResponse(code = 200, message = "OK"),
      @ApiResponse(code = 204, message = "No content"),
      @ApiResponse(code = 400, message = "Bad Request"),
  ...
  @ResponseBody
  public ResponseEntity<List<QuestionDto>> getGameQuestionsWithoutAnswer(@RequestHeader String code) {
    List<QuestionDto> questions = gameService.obtainQuestionsWithoutAnswer(code);
    return ResponseEntity.ok(questions);
  }

  @GetMapping(value = "/v1.0/game/code/{code}/all", produces = MediaType.APPLICATION_JSON_VALUE)
  @ApiOperation("get all the questions of a specific game.")
  @ApiResponses(value = {
      @ApiResponse(code = 200, message = "OK"),
      @ApiResponse(code = 204, message = "No content"),
      @ApiResponse(code = 400, message = "Bad Request"),
  ...
  @ResponseBody
  public ResponseEntity<List<QuestionDto>> getAllGameQuestions(@RequestHeader String code) {
    List<QuestionDto> questions = gameService.obtainAllQuestions(code);
    return ResponseEntity.ok(questions);
  }
